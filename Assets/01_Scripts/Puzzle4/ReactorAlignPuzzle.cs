...
public class ReactorAlignPuzzle : MonoBehaviour, IPuzzle
{
    ...
    void Update()
    {
        ...
        for (int i = 0; i < channels.Length; i++)
        {
            ...
            {
                // Si estaba intentando alinear y se sale del rango → daño al jugador
                if (c.holdTimer > 0f)
                {
                    PlayerHealth playerHealth = FindObjectOfType<PlayerHealth>();
                    if (playerHealth != null)
                    {
                        playerHealth.TakeDamage(1); // daño leve por fallo
                        Debug.Log("❌ ReactorAlign: canal fuera de rango, jugador pierde 5 de vida.");
                    }
                }
